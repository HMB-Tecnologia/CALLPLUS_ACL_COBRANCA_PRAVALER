...
{
    public partial class ListaCepForm : Form
    {
        public ListaCepForm()
        {
            _logger = LogManager.GetCurrentClassLogger();
            _campanhaService = new CampanhaService();
            _cepExpressService = new CepExpressService();
        ...
        private readonly CepExpressService _cepExpressService;
        ...
        {
            _campanhas = _campanhaService.Listar(null);
            cmbCampanha.PreencherComSelecione(_campanhas.Where(x => x.HabilitaCepExpress), campanha => campanha.Id, campanha => campanha.Nome);
        }
        ...
        private void CarregarGrid(bool buscaRapida)
        {
            ...
            if (ParametrosPesquisaValidos(buscaRapida))
            {
                ...
                dgResultado.DataSource = _cepExpressService.ListarExibicao(idRegistro, idCampanha, nome, ativo);
        ...
        {
            CadastroCepForm f = new CadastroCepForm("NOVO CADASTRO CEP", 0, _campanhas);
        ...
        private void IniciarEdicaoRegistro(int linha)
        {
            if (linha >= 0)
            {
                CepExpress cepExpress = new CepExpress();

                cepExpress.Id = (int)dgResultado.Rows[linha].Cells["ID"].Value;

                CadastroCepForm f = new CadastroCepForm("DETALHES DO CADASTRO CEP", cepExpress.Id, _campanhas);
        ...
        private void ListaCepForm_Load(object sender, System.EventArgs e)
        {
            try
            {
